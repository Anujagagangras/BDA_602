...
def import_data():
    ...
    df = pd.read_table('./results/dataset.csv')
...
def naive_bayes_model(X_train, y_train, X_test, y_test):
    ...
    print(
        "Root Mean Squared Error:",
        np.sqrt(metrics.mean_squared_error(y_test, y_pred_NB)),
    )
...
def mean_of_response(df, predictors, response):
    ...
    for i in range(0, len(predictors)):
        ...
        total_count = len(df_copy[response])

        for i_bin in range(bin_size):
            bin_data = df_copy[
                (bins_edges[i_bin + 1] >= df_copy[predictors[i]])
                & (bins_edges[i_bin] < df_copy[predictors[i]])
            ]
            ...
            bins_weighted_mean_squared_diff[i_bin] = np.square(
                (count[i_bin] / total_count) * (np.mean(bin_data[response]) - pop_mean)
            )
        ...
        fig.update_yaxes(title_text=f"Response - {response}", secondary_y=True)
    ...
    MOR_table = MOR_table.style.set_properties(
...
def Brute_Force_cont_cont(df, continuous, response):
    ...
    for a, b in combinations(continuous, 2):
        ...
        Brute_Force_Table["Weight"] = (
            Brute_Force_Table["Bin_Count"] / df[response].count()
        )
        ...
        for x in continuous_predictors:
            for y in continuous_predictors:
                if x != y and test_cont < 1:
                    ...
                    path = join(dirname(abspath(__file__)), "/results/Project_html", name)
                    fig.write_html(
                        path,
...
def main():
    ...
    path = join(dirname(abspath(__file__)), "/results/Project_html")
    ...
    continuous_predictors, categorical_predictors = create_list_of_cat_con_predictors(
        df_new_pred, predictors
    )
    cont_cont_correlation_plot, cont_cont_correlation_table = correlation_cont_cont(
        df_new_pred, continuous_predictors
    )
    ...
    with open("./results/final.html", "w+") as file:
        file.write(
